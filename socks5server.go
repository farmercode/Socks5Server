...
var ServerCfgInfo ServerConfig //配置文件信息
...
//插件
func StringInArray(s string,stringArray []string) bool{
    if len(stringArray) == 0 {
        return false
    }
    for _,targetString := range stringArray {
        if(strings.EqualFold(strings.Trim(targetString," \t\n"),s)){
            return true
        }
    }
    return false
...
func main() {
    ...
    error := config.LoadConfig(configPath,&ServerCfgInfo)
    if error != nil {
        fmt.Println("config file ",configPath," error :",error.Error())
        return
    }
    log.Println("Listen:\t",ServerCfgInfo.Listen)
    log.Println("WhiteList:\t",ServerCfgInfo.WhiteList)
    if len(ServerCfgInfo.Listen) == 0 {
        fmt.Println("Please config your listen information in ",configPath)
        return
    }

    LISTEN = ServerCfgInfo.Listen
    ListenAndServe()
}
